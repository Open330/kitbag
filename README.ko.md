...
나뉜 폭은 보이는 것보다 좁습니다. 먼저 잡을 것은 `status`입니다 — 추적
중인데 파일이 없는 경로를 이미 이름 대고, 필터가 몇 개를 뺐는지도 셉니다.
`tracked`는 **설정을 쓰인 그대로 읽어주는 것**입니다. 파일마다 한 줄이
아니라 트랙마다 한 줄이라, `~/.envs/*.env`가 있는 기계에서는 서른다섯 줄과
세 줄의 차이가 납니다.

정말로 다른 질문인 건 `catalogue`입니다. 이건 **아직 지키지 않는 것**을
어디서 찾을지에 대한 규칙이니까요.
